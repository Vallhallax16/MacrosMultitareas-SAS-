/* ------------------------------------------------------------------ *
 * Bundle: t001_fcmp_datosgenerales
 * Source: MacroFuncDatosGenerales.sas  (%DECLARAR_FUNCIONES, verbatim)
 *
 * The macro below is copied unchanged from the repo. It uses PROC FCMP
 * to compile the user-defined functions ArmarMDY / EquivalenciaMes /
 * StringAFecha into a function library. The caller at the bottom is the
 * only added code: it declares the functions into WORK, points CMPLIB at
 * them, and runs StringAFecha over a few sample date strings so you can
 * see the compiled functions in action.
 * ------------------------------------------------------------------ */
...
/* ---- caller: compile the functions, then convert a few sample dates ---- */
%DECLARAR_FUNCIONES(Libreria = WORK);

OPTIONS CMPLIB = WORK.FUNC;

DATA fechas_convertidas;
	LENGTH origen $20;
	INPUT origen $20.;
	fecha = StringAFecha(origen);
	FORMAT fecha DATE9.;
DATALINES;
20240115
20231231
...
PROC PRINT DATA = fechas_convertidas NOOBS;
	TITLE "StringAFecha: cadenas numericas y con guion -> fecha DATE9.";
RUN;
TITLE;
